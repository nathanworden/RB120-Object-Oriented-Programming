...
module Display
  ...
  def self.puts_headings(msg)
    puts " #{msg} ".center(SCREEN_WIDTH, '=')
  end
  
  def self.puts_subheadings(msg)
    puts " #{msg} ".center(SCREEN_WIDTH)
  end
...
class RPSGame
  ...
  def players_choose
    return flase unless human.choose
  ...
  def determine_winner
    ...
    winner&.track_winning_move # &. safe navigation, so only returns if not nill
    winner
  end
